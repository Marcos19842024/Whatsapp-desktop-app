...
class RemindersData {
    ...
    prepareClientsVacunas(rows) {
        return rows.reduce((acc, cell, index) => {
            try {
    ...
    generateMessagesVacunas(clientes) {
    ...
    generateMessagesCitas(clientes) {
        clientes.forEach(cliente => {
            const todasLasCitas = cliente.todasLasCitas;
            if (!todasLasCitas || todasLasCitas.length === 0) {
                return;
            }
            const primeraFecha = todasLasCitas[0].fecha;
            const mismoDia = todasLasCitas.every((cita) => cita.fecha === primeraFecha);
            ...
            delete cliente.todasLasCitas;
        });
    }
    // Procesar servicios
    procesarServicio(tipoVisita, asunto, estado) {
        const servicios = [];
    ...
    procesarServicioEstetica(asunto, estado) {
        ...
        if (tieneCP)
            servicios.push('corte');
        if (tieneB || tieneBaño)
            servicios.push('baño');
        if (tieneBM)
            servicios.push('baño medicado');
        if (tieneTransporte)
    ...
    listarCitasParaMascota(citas, incluirFecha = true) {
        if (citas.length === 0)
            return '';
        if (citas.length === 1) {
            ...
            for (let i = 0; i < citas.length; i++) {
                const servicio = this.procesarServicio(citas[i].tipo_visita, citas[i].asunto, citas[i].estado);
                if (i === 0) {
    ...
    describirCita(cita, incluirFecha = true) {
        const servicio = this.procesarServicio(cita.tipo_visita, cita.asunto, cita.estado);
        return this.construirDescripcionCita(cita, servicio, incluirFecha, false);
    }
    construirDescripcionCita(cita, servicio, incluirFecha, esSegundaOMas) {
        let descripcion = '';
    ...
    // Helpers
    extraerNombreClinica(nombreCompleto) {
        if (!nombreCompleto)
            return '';
    ...
    formatFechaParaMensaje(fechaStr) {
...
exports.default = RemindersData;
//# sourceMappingURL=remindersData.js.map